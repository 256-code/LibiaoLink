...
migrate: 完成，本次执行 9 个迁移
```

再次执行输出 `migrate: 数据库已是最新（已执行 9 个迁移，无漂移）`；随后执行种子（示例）：
...
- **空库迁移成功**：按「迁移命令」执行；迁移后 `schema_migrations` 9 行、业务表 18 张 + 迁移记录表 1 张（h1 起含 0007 的 4 张身份 / 角色表）。
