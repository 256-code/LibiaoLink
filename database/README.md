...
| `seeds/role-permissions.mjs` | 种子 #6b：六角色 × 权限位矩阵（h6 · Push 95，h8 补 `calendar.manage`，M6-01 ~ M6-03 补 `report.view` / `report.fill` / `issue.view` / `issue.manage`（Push 154 · 27 → 31 键）；键唯一来源 = 契约 `PERMISSION_KEYS`，移除键会删除 —— 权限吊销必须生效） |
