...
| `migrations/0025_priority_three_levels.sql` | 紧急重要度收敛三档（M3-07 · Push 163 · px 线）：口径以前端页面为准 —— 契约 `PrioritySchema` 原四象限（重要且紧急 / 紧急但不重要 / 重要不紧急 / 不紧急不重要）与页面三档「高 / 中 / 低」不一致，本迁移把 `tasks.priority` 存量值折算（重要且紧急 → 高；重要不紧急 / 紧急但不重要 → 中；不紧急不重要 → 低）；`priority` 是 text 无 CHECK / 无枚举，**无 DDL**、只改值，折算有损不回滚 |
