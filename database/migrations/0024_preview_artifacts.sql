...
--   shared/src/modules/audits.ts（AUDIT_ACTIONS 含 preview —— D2-07 预览计入查看 / 下载审计）。
-- 口径：
...
--   5. 审计：ck_audit_logs_action 由八值扩为九值（新增 preview，D2-07）；只扩值、不改列，回滚见文末。
...
--     add constraint ck_audit_logs_action check (action in ('create','update','delete','progress','complete','advance','rollback','deny'));
--   drop table preview_artifacts;
...
-- 审计动作扩值（D2-07 预览计入查看 / 下载审计）：CHECK 不支持追加取值 → drop + 重建；
...
alter table audit_logs add constraint ck_audit_logs_action
  check (action in ('create','update','delete','progress','complete','advance','rollback','preview','deny'));