...
- 剩余（M4-05 收口项）：**预览产物对象清理**（`previews/{contentHash}/` 前缀）；另 `GET /files/{id}/versions/{versionId}/download-url`（下载切片，契约已在 `shared/`）仍未实现 —— M4-05 本体不含下载端点。
