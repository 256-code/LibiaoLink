...
- `0001_baseline.sql` 覆盖 §2.3 全部 10 张表与索引，另补两处（文件头已注明）：
  ...
  2. 补齐 4 条关系外键：`file_versions.change_request_id`、`files.current_version_id`、`files.task_id`、`tasks.change_ref`（**后者已随 `0019_task_change_refs.sql` 移除** —— 变更关联改多值 `change_refs uuid[]`，Postgres 无数组外键；`change_requests` 只追加、无删除路径，原 `on delete set null` 不会被触发）。
