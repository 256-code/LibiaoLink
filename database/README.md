...
| `migrations/0009_projects_soft_delete.sql` | 项目软删（h2 · M2-01 / A5；**删除语义已由 0035 改为物理删，Push 190**）：`projects.deleted_at` / `deleted_by` + 活跃行局部索引 `ix_projects_active_updated`（列表 / 详情 / facets 统一过滤软删行） |
...
| `migrations/0035_project_hard_delete.sql` | 项目硬删口径（业务定案 2026-09-24「删除要硬删不要软删」「同一个编号删除了 再建一个编号都不可以」· Push 190 · 代做 wmj 线领域与 lan 线迁移，请 wmj / lan 复核）：只改注释 + 一次存量清理，**不动结构** —— ① 按外键依赖序物理清掉存量软删项目（同样 14 条 delete：issue_events → task_events → file_versions → files → change_requests → issues → daily_reports → tasks → node_requirements → project_nodes → project_stages → project_members → project_stakeholders → projects；沙箱库 `projects` 175 → 108 行、`tasks` 278 → 85、`project_nodes` 3325 → 2052、`project_stages` 1575 → 972，审计历史保留）；② `projects` 表与 `deleted_at` / `deleted_by` 列注释改为「兼容列（Push 190 起新数据恒 null、读面仍过滤）」；③ `projects_code_key` 约束注释改为「删除即释放编号 → 同编号可再建」。`seq_no` 仍不回收。 |
