...
> **本表状态（PR-7 · Push 141 · M4-04 变更写入）**：**契约零改动**（`ChangeIntentBody` / `ChangeRequest` / `UploadCompleteResponse.changeRequest` 等字段面已随 Push 133~137 契约切片就位；`npm run check` 零漂移、生成物未重出）—— 实现侧放开上传入口 `intent=change`（目标须 final / changed；非该状态 409 `FILE_STATE_INVALID`）与「定档后回溯 = 变更流」，删除切片守卫 `intent_change_not_open` / `change_flow_not_open`；变更读面（`GET /projects/{id}/change-requests` / `GET /change-requests/{id}`）与统计（A4-17）随 M4-04 下一切片，预览（M4-05）随后。
