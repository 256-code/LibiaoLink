...
## 规则引擎（S6·automation · M5-01：规则模型 + 条件求值 + 回放器）
...
- 触发窗口与幂等：`resolveScheduleFire`（T_MINUS_1 / SAME_DAY / T_PLUS_1 走日历 `evaluateOffset`，顺延开关与方向由调用方按日历设置注入；WEEKLY 取业务日所在周的周一）—— `windowKey` = 触发日（T±N / 当天）或 ISO 周键（WEEKLY）；幂等执行键 = `dedupeKey(规则码, 实体, 窗口)`（v0.2 §6.2；实体 = 非分组规则的任务 id / 分组规则的收件人 id）。
- 回放器（`replayRules`）：业务日 + 任务数据集 + 日历 → 应发送清单（逐规则 × 逐任务：窗口 → 条件 → 动作 / 收件人 → 合并 → 幂等），`details` 逐条给命中与跳过原因（`window_mismatch` / `not_matched` / `recipient_missing` / `template_missing` / `duplicate` / `disabled`）—— 供 C2-12「规则可解释」与金标回归；事件型规则在回放中按「任务处完成态 = 完成事件已发生」近似（真实事件由 outbox 承载 · M5-02）。
