...
| `migrations/0028_user_preferences.sql` | 用户偏好表（A4 / A24 · Push 169 · px 线）：`user_preferences`（`user_id uuid` 主键 → `users(id)` on delete cascade、`prefs jsonb not null default {}`、`updated_at timestamptz not null default now()`、CHECK `ck_user_preferences_prefs_object jsonb_typeof(prefs) = object`；**一人一行**、单用户单写者无 version）—— 偏好键 `taskTableHiddenColumns`（A4 列显隐）/ `homeSavedFilters`（A24 常用筛选，≤ 20 组）；界面状态、不入审计 |
...
- identity/user-preferences（A4 / A24 · Push 169 · px 线）：`0028_user_preferences.sql` 落 `user_preferences`（`user_id` 主键 → `users(id)` 级联删；`prefs` jsonb 默认 `{}` + 对象形状 CHECK；`updated_at` 写入时刷新）—— 端点 `GET / PATCH /api/v1/users/me/preferences`（PATCH 合并语义，只读写会话 actor 自己那一行）；无记录 = 默认值 + `updatedAt: null`；读侧坏数据规范化（丢弃 / 截断）在服务层做，不动库结构。
