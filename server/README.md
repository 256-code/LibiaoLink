...
- `GET / PATCH /api/v1/users/me/preferences`（A4 / A24 · Push 169）：**用户偏好** —— 一期两个键：任务表列显隐 `taskTableHiddenColumns`（A4）与首页「常用筛选」`homeSavedFilters`（A24，`SavedHomeFilter[]`，≤ 20 组 / 名称 ≤ 20 字）。落 `user_preferences`（迁移 `0028_user_preferences.sql`；一人一行、`prefs` jsonb、单用户单写者无 version）：GET 无记录 = 默认值 + `updatedAt: null`；PATCH 为**合并语义**（只传变更键、数组键整体替换、未声明键原样保留 —— catchall 前向兼容），写路径挂 `CsrfGuard`；**只读写会话 actor 自己那一行**（路径不接受用户 id，跨设备同账号可见、换账号互不可见）；读侧坏数据规范化在服务层做（坏条目丢弃 / id 去重 / 名称截断 20 / 非法日期置 null），偏好属界面状态**不写审计**。实现：`src/modules/identity/user-preference.repository.ts` + `user-preference.service.ts`（`users.controller.ts` 加两条路由，`identity.module.ts` 注册）；单测 `test/user-preferences.test.ts`（7 例）。
