...
- 字典读（C9-01 / C9-03）：GET /api/v1/dicts 与 /{type}，默认只回 `enabled=true`；`includeDisabled=true`（**兼容参数**：Push 173 起删除 = 物理删行，一期不再产生停用项）需 dict.manage；未知类型 404；响应 `updatedAt` 取类型版本（条目变更 touchType），供前端做缓存刷新。
- 字典写（C9-02；2026-09-23：Push 168 权限修订 + Push 173 删除口径修订）：POST `{type}/items` 按类型分权 —— **region 登录即可**（全站共享的公共标签；重复码仍 409）、其余类型 dict.manage；PATCH（改名 / 排序；`enabled` 为兼容字段）与 **DELETE `{type}/items/{code}`（删除 = 物理删行：事务内取快照 → 删行 → touchType → 审计 action=delete → 返回删除后的整个字典；未知条目 404）** 均仅 dict.manage（治理权不放开）；同类型内码唯一（重复 409 DICT_ITEM_EXISTS）；删除**不影响存量数据**（项目按原码 / 原名渲染）、且**删除无记忆** —— 同码可重新新增、按全新条目处理；响应为更新后的整个字典（前端直接替换缓存）。
...
- `server/test/admin-audit.test.ts`（18 例）：diff / 稳定序列化、路径解析与越权判定、DictService 物理删行（审计 action=delete）/ 删除后同码可重建 / 删除未知条目 404 不写审计 / 码唯一 / 兼容 PATCH 与无变更不留痕、AuditService 写入 / 检索 / 人员快照 / 失败吞错。
