...
server/
  src/
    ...
    modules/file/         # 文件与版本（S7·file · M4-01 上传管道 + M4-02 版本 / 定档 / 回溯 / 回收站 + M4-03 文件库查询与多态关联 + M4-04 变更（申请即通过））：发起上传 / 分片预签名 / 断点续传 / 完成落版本（写 file_links）/ 取消 / 详情 / 版本链 / 定档 / 回溯 / 回收 / 恢复 / 彻底删除 / 项目文件库列表（筛选 / 关键字 / 排序 / 分页 / 双向跳转）（变更申请即通过已随 M4-04 接入；预览 M4-05 数据层已落地：`preview_artifacts` · 迁移 `0022`，转换器 / 读面随后续切片）
...
| `npm run check:db-schema` | Drizzle schema ↔ 实际库（需先 build；当前 30 表 / 296 列 / 93 索引·唯一 / 91 CHECK） |
...
- 迁移是唯一 DDL 来源（`database/migrations/`，只追加）；`src/db/schema/` 的 Drizzle 定义必须与迁移后的最终结构一致（当前 0001 ~ 0020）。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/admin-audit.test.ts` 15 例 + `check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）+ `check:permission-matrix`（6 角色 / 74 条目 / 27 键）。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/calendar-rules.test.ts` 17 例 + `test/calendar-service.test.ts` 12 例 + `check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）+ `check:permission-matrix`（6 角色 / 74 条目 / 27 键）。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/task-order.test.ts` 4 例 + `test/task-service.test.ts` 19 例 + `check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）。
...
- 覆盖范围提示：本卡只落「上传管道」（发起 / 分片 / 状态 / 完成 / 取消 + 过期清理）；**文件详情 / 版本链 / 定档 / 回溯 / 回收站 / 恢复 / 彻底删除已随 M4-02（PR-5）接入同一控制器**，变更（M4-04）写入已接入同一控制器（读面 / 统计随下一切片），预览（M4-05）数据层已落地（`preview_artifacts` · 迁移 `0022`），转换器与读面随下一切片。U22 断言已随 M4-04 切换（change 目标门禁：非定档 → 409 `FILE_STATE_INVALID`）。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/file-service.test.ts` 66 例（服务层替身直测，含 M4-03 / M4-04 变更写入）；`check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）；`check:boundaries` / `check:permission-matrix` 覆盖新增文件与权限键用法。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/file-service.test.ts` 66 例（服务层替身直测，含 M4-04 变更写入）+ `test/change-read.test.ts` 7 例（变更读面：列表 / 详情 / 可见性 / 非法输入）+ `test/schema-literals-parity.test.ts` 3 例（库侧枚举字面量 ↔ 契约值集：审计动作 / 预览通道 / 预览状态）；`check:boundaries`（137 文件 / 525 依赖 / 0 违规）/ `check:permission-matrix`（6 角色 / 74 条 / 27 键）/ `check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）覆盖新增文件与迁移。
...
- 迁移：`database/migrations/0022_preview_artifacts.sql`（只追加）—— `preview_artifacts`（三元组缓存键 `content_hash + pipeline_version + target` **唯一**（ADR-007 / D2-06）+ `(version_id, target)` / `file_id` 反查索引 + **5 CHECK**：取值 / 状态 / `ready` 成对 / `failed` 成对 / `error ≤ 500`）+ `ck_audit_logs_action` 由八值扩九值（新增 `preview`，D2-07）。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/schema-literals-parity.test.ts` 3 例（库侧枚举字面量 ↔ 契约值集）；`check:db-schema`（**Push 150：30 表 / 296 列 / 93 索引·唯一 / 91 CHECK**）；`check:boundaries`（137 文件 / 525 依赖 / 0 违规）。
- 真机核对（本机库已执行 `0022`）：审计 CHECK 实值含 `preview`（`action = ANY (ARRAY['create', …, 'preview', 'deny'])`）、`preview_artifacts` 12 列 / 4 索引 / 5 CHECK、应用角色 `libiaolink_api` 可写 `preview` 审计与新表（由 `roles/0001` default privileges 自动授权，无需额外 GRANT）。
