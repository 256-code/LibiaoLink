...
server/
  src/
    ...
    modules/file/         # 文件与版本（S7·file · M4-01 上传管道 + M4-02 版本 / 定档 / 回溯 / 回收站 + M4-03 文件库查询与多态关联 + M4-04 变更（申请即通过））：发起上传 / 分片预签名 / 断点续传 / 完成落版本（写 file_links）/ 取消 / 详情 / 版本链 / 定档 / 回溯 / 回收 / 恢复 / 彻底删除 / 项目文件库列表（筛选 / 关键字 / 排序 / 分页 / 双向跳转）（变更申请即通过已随 M4-04 接入；预览 M4-05 数据层与转换队列已落地：`preview_artifacts` + `ck_audit_logs_action` 十值 · 迁移 `0027`，outbox `preview.job` 领取器 / 转换沙箱客户端 / 三元组幂等 / 失败降级 · 迁移 `0028`；读 API 随 PR-11）
...
- **触发点**：**定档预生成（P1）** —— `finalizeFile` 同事务投当前版本的 `preview.job`（只投一期真能出产物的通道：图片 → `image`；PDF / Office（含 xlsx）→ `pdf`；判不出类型不投），去重键 = 三元组；**其余按需懒生成** = 读取侧（PR-11）按 `not_ready` 幂等补投；上传完成**不**投（draft 可能被替换，白转）。
...
- **审计口径**（D2-07）：**生成侧不写审计** —— 转换是系统内部副作用（产物状态由 `preview_artifacts` 留痕、任务进度由 `outbox_events.attempts` / `last_error` 留痕），审计只记**用户访问**（`object_type = file` + `action = preview`，且只对 `ready` 的读取写，随 PR-11）；否则一次预览会写两条、把「查看 / 下载」审计计数翻倍。
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/preview-queue.test.ts` **24 例** + `test/file-service.test.ts` **68 例**（含定档预生成投递）；`check:boundaries`（152 文件 / 610 依赖 / 0 违规）；`check:db-schema`（**Push 167：33 表 / 341 列 / 107 索引·唯一 / 108 CHECK**）。
