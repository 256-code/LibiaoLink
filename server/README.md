...
| `npm run check:db-schema` | Drizzle schema ↔ 实际库（需先 build；当前 33 表 / 340 列 / 106 索引·唯一 / 108 CHECK） |
...
- 剩余：`GET /files/{id}/versions/{versionId}/download-url`（下载切片，契约已在 `shared/`）仍未实现 —— M4-05 本体不含下载端点；**产物清理已随 PR-12 落地**（见下「M4-05e 产物清理」）。
