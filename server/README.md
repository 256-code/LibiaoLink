...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/file-service.test.ts` 61 例（服务层替身直测，含 M4-03）；`check:db-schema`（27 表 / 265 列 / 77 索引 / 75 CHECK）；`check:boundaries` / `check:permission-matrix` 覆盖新增文件与权限键用法。
