...
try {
  ...
  check("A3", "created 字段：阶段取模板阶段 / 状态 pending / 进度 0 / 负责人缺省 = 项目经理 / 重要度 = 调用方给", "design / pending / 0 / [" + admin.id + "] / 中", truncate({ stageKey: created1[0]?.stageKey, status: created1[0]?.status, progress: created1[0]?.progress, ownerIds: created1[0]?.ownerIds, priority: created1[0]?.priority }), created1.every((item) => item.stageKey === NODE_STAGE && item.status === "pending" && item.progress === 0 && item.priority === "中") && (created1[0]?.ownerIds ?? []).join(",") === admin.id);
  ...
  check("B2", "描述 / 英文名 / 阶段取节点库现值（客户端带的标题被覆盖）+ sourceNodeId 回传 + 负责人缺省 = 项目经理", nodeTitle2 + " / null / design", truncate({ title: bAdd.body?.title, titleEn: bAdd.body?.titleEn, stageKey: bAdd.body?.stageKey, sourceNodeId: bAdd.body?.sourceNodeId, ownerIds: bAdd.body?.ownerIds }), bAdd.body?.title === nodeTitle2 && bAdd.body?.titleEn === null && bAdd.body?.stageKey === NODE_STAGE && bAdd.body?.sourceNodeId === n2.id && (bAdd.body?.ownerIds ?? []).join(",") === admin.id);
