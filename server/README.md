# server/ · 后端工程（g4 骨架 · g6 会话后端化 · h1 identity/org · h2 project · h3 流程节点 · h4 task · h5 PoC-9 · h6 权限矩阵 · h7 字典与审计 · h8 工作日历 · w2 任务落库口径 · 存储接入 · M4-01 上传管道 · M4-02 版本与回收站）
...
server/
  src/
    ...
    modules/file/         # 文件与版本（S7·file · M4-01 上传管道 + M4-02 版本 / 定档 / 回溯 / 回收站）：发起上传 / 分片预签名 / 断点续传 / 完成落版本 / 取消 / 详情 / 版本链 / 定档 / 回溯 / 回收 / 恢复 / 彻底删除（变更按 M4-04 接入）
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/file-service.test.ts` 55 例（服务层替身直测，含 M4-02 生命周期）；`check:boundaries` / `check:permission-matrix` 覆盖新增模块与权限键用法。
...
- 门禁（不连库，随 `npm test` 与 CI 常跑）：`test/file-service.test.ts` 55 例（服务层替身直测）；`check:boundaries` / `check:permission-matrix` 覆盖新增模块与权限键用法。
