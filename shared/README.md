...
| 审计动作 | `AUDIT_ACTIONS`：create / update / delete / progress / complete / advance / rollback / **preview**（契约切片 · M4-05 前置；D2-07：预览计入查看 / 下载审计 —— 库侧 `ck_audit_logs_action` 现八值，随 M4-05 迁移扩值）/ **download**（Push 160 定案；A4-10：离线下载受权限控制并记日志 —— 对象类型 file + metadata 记 versionId；库侧随 M4-05 迁移一次扩至十值） / deny |
